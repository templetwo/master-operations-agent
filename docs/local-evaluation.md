...
Requests use temperature 0, seed 0, context 8192, output budget 768, `think: false`, and non-streaming responses. These are requested settings, not proof of deterministic inference or proof that the backend applies every option. The client records request hashes, the returned response hash, bounded visible content, wall time, and available daemon timing/token counters. It does not retain a separate reasoning field. Missing counters remain unknown. Time to first token is unmeasured.
