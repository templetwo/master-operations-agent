...
The development comparator retains its default limit of 61 inference requests, including the initial protocol request. v0.7 permits an explicit integer `max_calls` between 1 and 145 for a separately registered campaign; the chosen cap appears in provider metadata. This changes no request settings or automatic retry behavior. Each request is bounded in size, output tokens, and socket timeout. Unknown usage counters remain unknown. Warmup usage is in `warmup.json`; scored usage is separate. The tool does not report an estimated cost as an actual charge. Consult the provider account for billed amounts.
...
A subsequent [preregistered four-arm guidance experiment](guidance-experiment.md) completed 160 scored calls plus four warmups. Accepted advisories were unchanged 0/10, output-only 0/10, policy-only 5/10, and both 4/10. [Full results and diagnostic audit](../receipts/v0.6/guidance-live/README.md) separate output format from content selection. The default prompt/provider and release validator remain unchanged; no cloud configuration is promoted by these development results.
