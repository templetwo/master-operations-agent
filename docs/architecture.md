...
V0.5 adds a separate, explicitly invoked DeepSeek comparison command. It generates cases through the trusted simulator exporter and sends those synthetic observations to the fixed official HTTPS API. It cannot accept imported snapshots or enable cloud in the dashboard. Its literal dotenv loader requires an explicit path, never executes file contents, and never stores the key in receipts. JSON-mode responses pass through the same candidate validator. It uses a mutable API alias rather than claiming a local artifact digest. Actual inference succeeded after the credential update. The first configuration completed all evidence reads but produced 0/10 accepted advisories; strict validation withheld every candidate. See [DeepSeek setup](deepseek.md).

The v0.6 research additions record failure stage/category/code separately from the existing terminal reason. The optional trusted `Agent(system_prompt=...)` constructor argument supports preregistered static guidance experiments; imported observations cannot set it, and no arbitrary-prompt override is exposed by the dashboard. Offline diagnostics import no runtime policy/oracle helpers and never authorize output. Their labels remain project-authored expectations. The four-arm experiment pins exact prompts and source hashes, rotates arm order, and records all results without automatic promotion. See [experiment contract](guidance-experiment.md) and [diagnostic contract](diagnostics.md).
