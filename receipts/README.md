...
The `v0.5/` directory contains DeepSeek authentication-attempt receipts and current implementation validation. See its [run index](v0.5/README.md). `v0.4/` contains schema-order diagnostics and corrected local-model comparisons. `v0.3/` preserves earlier model comparisons affected by the schema serialization bug. `v0.2/` preserves time-series validation and `latest/` preserves the original v0.1 bootstrap run. Run from the repository root:
